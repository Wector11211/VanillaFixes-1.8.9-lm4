...
import net.labymod.v1_8.client.input.InputHandler;
...
public abstract class F3CommandsFixMixin {
	...
	private void runTickKeyboard()
	{
		while (Keyboard.next())
		{
			...
			InputHandler.fireKeyboardInput(false);
			// ==
...
//			boolean flag = this.debugRenderer.toggleChunkBorders();
		...
		{
//			TODO: figure out why canCommandSenderUseCommand calls isRidingHorse
//			EntityPlayerSP player = this.thePlayer;
//			boolean canUseCommand = player.canCommandSenderUseCommand(2, "");
//			displayChatMessage(String.valueOf(canUseCommand));
//			if (!this.thePlayer.canCommandSenderUseCommand(2, "gamemode"))
//			{
//				this.debugFeedbackTranslated("vanillafixes.debug.creative_spectator.error");
//			}
			if(false) {}
			else if (this.playerController.isInCreativeMode())
			{
	...
	@Inject(method = "runTick", at = @At(value = "INVOKE", target = "Lnet/minecraft/profiler/Profiler;endStartSection(Ljava/lang/String;)V", shift = Shift.AFTER), slice = @Slice(from = @At(value = "CONSTANT", args = "stringValue=keyboard")), cancellable = true)
